...
class Color(object):
    ...
    def color_re(cls):
        return re.compile(
            r"\[(?P<dark>dark )?(?P<color>%s)\]" % "|".join(cls.colors()),
            re.MULTILINE,
        )
...
def strip_colors(text):
    """Strip out the color tags.

    This is used so that other functions can determine the real text line
    lengths.

    Args:
        text (str): Text to strip color tags from.
